    ...
    extension GameScene {
      ...
      // Touch handling:
      func tapOrClick(location: CGPoint) {
        // Make player jump:
        player.pb.applyImpulse(CGVector(dx: 0, dy: 50))
        
        // Reset player on label click:
        if nodes(at: location).first?.name == names.resetLabel {
      ...
      override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        tapOrClick(location: touches.first!.location(in: self))
      }
      #elseif os(OSX)
      override func mouseDown(with event: NSEvent) {
        tapOrClick(location: event.location(in: self))
      }
      #endif
